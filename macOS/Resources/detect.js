 ...
 * detect.js — Node 检测器
 * ------------------------------------------------------------------
 * 复用 citation-review-for-word-github 的 JS 引擎：
 *   - rules-apa7.js  （APA 7 风格包）
 *   - rules-loader.js（规则加载器 RulesManager）
 *   - taskpane.js 中的纯算法函数（runDetection 等，本文件内内嵌）
 *
 * 输入：doc.json（由 export-word.js 从 Word 导出）
 *       { paragraphs: [ { text, italicRanges, entireItalic }, ... ] }
 * 输出：problems.json（问题列表，供 rumps 面板展示）
 *       { comments: [...], stats: {...} }
 *
 * 用法：node detect.js <doc.json> <problems.json>
 * ================================================================== */
...
function loadJS(file) {
  ...
  // 在全局上下文执行（模拟 <script> 标签）
  (0, eval)(code);
}
loadJS('rules-apa7.js');
loadJS('rules-loader.js');

/* ==================================================================
 * 以下为从 taskpane.js 提取的纯算法函数（不依赖 Office.js）
 * ================================================================== */
...
/* ==================================================================
 * runDetection — 主检测（从 taskpane.js 提取，无 Office 依赖）
 * ================================================================== */
function runDetection(bodyText, refBlocks){
  ...
  for(const c of rawCites){
    ...
    citeKeyItems.push({ key: keys[0], allKeys: keys, authorsRaw: c.authorsRaw, authors: c.authors, etal: c.etal, year: c.year, raw: c.raw, start: c.start, end: c.end });
  }
 ...
 * ================================================================== */
function main(){
  const [docJsonPath, outJsonPath] = process.argv.slice(2);
  if(!docJsonPath || !outJsonPath){
    console.error('用法: node detect.js <doc.json> <problems.json>');
    process.exit(1);
  }
  ...
  const { body, refsParagraphs } = splitBodyAndReferences(paragraphs);
  const refBlocks = refsParagraphs.map(p => ({
    text: p.text,
    rawText: p.text,
    italics: p.entireItalic ? [[0, p.text.length]] : (p.italicRanges || []),
    html: escapeHtml(p.text)
  }));

  const result = runDetection(body, refBlocks);
  // 输出精简后的问题列表（供面板展示）
  const out = {
  ...
  console.log(`检测完成：共 ${out.comments.length} 处问题。`);
}
